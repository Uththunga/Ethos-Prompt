import { auth } from '@/config/firebase';
import type { EmailTemplate, EmailTemplateType } from '@/types';

interface EmailTemplatesListResponse {
  success: boolean;
  templates: EmailTemplate[];
}

interface EmailTemplateResponse {
  success: boolean;
  template: EmailTemplate;
}

interface DeleteResponse {
  success: boolean;
}

interface ListTemplatesParams {
  type?: EmailTemplateType;
  onlyActive?: boolean;
}

interface CreateEmailTemplatePayload {
  name: string;
  type: EmailTemplateType;
  subject: string;
  bodyHtml: string;
  description?: string;
  bodyText?: string;
  variables?: string[];
  isActive?: boolean;
}

function buildBaseUrl(): string {
  const isEmulators = import.meta.env.VITE_ENABLE_EMULATORS === 'true';
  const explicitApi = (import.meta.env.VITE_API_URL || '').toString().trim();
  const projectId = import.meta.env.VITE_FIREBASE_PROJECT_ID as string | undefined;
  const region = 'australia-southeast1';
  const functionName = (import.meta.env.VITE_FUNCTIONS_HTTP_API_NAME as string) || 'httpApi';

  if (isEmulators) {
    const providedBase =
      (import.meta.env.VITE_FIREBASE_FUNCTIONS_URL as string) ||
      (projectId ? `http://localhost:5001/${projectId}/${region}` : 'http://localhost:5001');
    return providedBase.endsWith(`/${functionName}`)
      ? providedBase
      : `${providedBase}/${functionName}`;
  }

  if (explicitApi) {
    return explicitApi.replace(/\/$/, '');
  }

  if (typeof globalThis !== 'undefined' && (globalThis as any).location?.origin) {
    return (globalThis as any).location.origin as string;
  }

  return '';
}

const baseUrl = buildBaseUrl();

async function getAuthHeader(): Promise<Record<string, string>> {
  const user = auth.currentUser;
  if (!user) {
    throw new Error('User not authenticated');
  }
  const token = await user.getIdToken();
  return { Authorization: `Bearer ${token}` };
}

export const emailTemplatesAdminService = {
  async listTemplates(params: ListTemplatesParams = {}): Promise<EmailTemplatesListResponse> {
    const headers = await getAuthHeader();
    const search = new URLSearchParams();
    if (params.type) search.set('type', params.type);
    if (params.onlyActive) search.set('onlyActive', 'true');
    const query = search.toString() ? `?${search.toString()}` : '';
    const url = `${baseUrl}/api/email-templates${query}`;
    const res = await fetch(url, { headers });
    if (!res.ok) {
      throw new Error(`Failed to list email templates: HTTP ${res.status}`);
    }
    return res.json();
  },

  async getTemplate(id: string): Promise<EmailTemplateResponse> {
    const headers = await getAuthHeader();
    const url = `${baseUrl}/api/email-templates/${encodeURIComponent(id)}`;
    const res = await fetch(url, { headers });
    if (res.status === 404) {
      throw new Error('Email template not found');
    }
    if (!res.ok) {
      throw new Error(`Failed to get email template: HTTP ${res.status}`);
    }
    return res.json();
  },

  async createTemplate(payload: CreateEmailTemplatePayload): Promise<EmailTemplateResponse> {
    const headers = await getAuthHeader();
    const url = `${baseUrl}/api/email-templates`;
    const body: Record<string, unknown> = {
      name: payload.name,
      type: payload.type,
      subject: payload.subject,
      bodyHtml: payload.bodyHtml,
      description: payload.description,
      bodyText: payload.bodyText,
      variables: payload.variables ?? [],
      isActive: payload.isActive,
    };
    const res = await fetch(url, {
      method: 'POST',
      headers: { ...headers, 'Content-Type': 'application/json' },
      body: JSON.stringify(body),
    });
    if (!res.ok) {
      const text = await res.text().catch(() => '');
      throw new Error(`Failed to create email template: HTTP ${res.status} ${text}`);
    }
    return res.json();
  },

  async updateTemplate(id: string, updates: Partial<EmailTemplate>): Promise<EmailTemplateResponse> {
    const headers = await getAuthHeader();
    const url = `${baseUrl}/api/email-templates/${encodeURIComponent(id)}`;
    const body: Record<string, unknown> = {};

    if (updates.name !== undefined) body.name = updates.name;
    if (updates.description !== undefined) body.description = updates.description;
    if (updates.type !== undefined) body.type = updates.type;
    if (updates.subject !== undefined) body.subject = updates.subject;
    if (updates.bodyHtml !== undefined) body.bodyHtml = updates.bodyHtml;
    if (updates.bodyText !== undefined) body.bodyText = updates.bodyText;
    if (updates.variables !== undefined) body.variables = updates.variables;
    if (updates.isActive !== undefined) body.isActive = updates.isActive;

    const res = await fetch(url, {
      method: 'PATCH',
      headers: { ...headers, 'Content-Type': 'application/json' },
      body: JSON.stringify(body),
    });
    if (!res.ok) {
      const text = await res.text().catch(() => '');
      throw new Error(`Failed to update email template: HTTP ${res.status} ${text}`);
    }
    return res.json();
  },

  async deleteTemplate(id: string): Promise<DeleteResponse> {
    const headers = await getAuthHeader();
    const url = `${baseUrl}/api/email-templates/${encodeURIComponent(id)}`;
    const res = await fetch(url, {
      method: 'DELETE',
      headers,
    });
    if (res.status === 404) {
      throw new Error('Email template not found');
    }
    if (!res.ok) {
      const text = await res.text().catch(() => '');
      throw new Error(`Failed to delete email template: HTTP ${res.status} ${text}`);
    }
    return res.json();
  },
};
