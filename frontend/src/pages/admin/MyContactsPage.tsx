...
const isDueOrOverdue = (nextFollowUpAt: any): boolean => {
  const d = toDateValue(nextFollowUpAt);
  if (!d) return false;
  const now = new Date();
  return d <= now;
};
...
export const MyContactsPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchContacts = async () => {
      ...
      setError(null);
      try {
        const res = await contactsAdminService.listMyContacts({
          page,
          pageSize: PAGE_SIZE,
        ...
        if (!mounted) return;
        setError(e?.message || 'Failed to load my contacts');
      } finally {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-ethos-navy">My Contacts</h1>
          <p className="text-sm text-ethos-gray">
            Contacts where you are the owner, with emphasis on due and upcoming follow-ups.
          </p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 flex flex-wrap gap-4 items-center">
        <div className="flex-1 min-w-[200px]">
          <label className="block text-xs font-medium text-gray-500 mb-1" htmlFor="my-contacts-search">
            Search
          </label>
          <input
            id="my-contacts-search"
            type="text"
            value={search}
            onChange={(e) => {
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {loading && contacts.length === 0 ? (
                ...
                filtered.map((contact) => {
                  const due = isDueOrOverdue(contact.nextFollowUpAt);
                  return (
                    <tr
                      key={contact.id}
                      className={
                        due ? 'bg-amber-50 hover:bg-amber-100 cursor-pointer' : 'hover:bg-gray-50 cursor-pointer'
                      }
